...
func GetCreateFlags() []cli.Flag {
	return []cli.Flag{
		...
		cli.StringFlag{
			...
			Usage: "Aliyun Docker Daemon Insecure Registry",
		},
	}
}
...
func (d *Driver) Create() error {
	...
	if err := ssh.WaitForTCP(fmt.Sprintf("%s:%d", d.IPAddress, port)); err != nil {
		return err
	}
...
func (d *Driver) uploadKeyPair() error {
	...
	sshCli, err := ssh.NewClient(d.GetSSHUsername(), d.IPAddress, port, &auth)
	if err != nil {
		return err
	}

	command := fmt.Sprintf("mkdir -p ~/.ssh; echo '%s' > ~/.ssh/authorized_keys", string(d.PublicKey))
	output, err := sshCli.Run(command)
	...
	if strings.Contains(d.ImageId, "centos6") {
		output, err = sshCli.Run("chmod 644 /etc/sudoers; sed -r -i 's/^(Defaults\\s+requiretty)$/#\\1/' /etc/sudoers; chmod 400 /etc/sudoers")
		log.Debugf("enable sudo without tty with err, output: %v: %s", err, output)
	} else {
		output, err = sshCli.Run("route -n| grep -e '^172\\..*$'|awk '{print \"route del -net \"$1\" netmask \"$3\" dev \"$8|\"/bin/bash\"}'")
		log.Debugf("delete route rule with err, output: %v: %s", err, output)

		output, err = sshCli.Run("sed -r -i 's/^(up route add \\-net 172\\..*)$/#\\1/' /etc/network/interfaces")
